...
class ControllerStateCivil extends Controller
{
    public function create(Request $request, Response $response)
    {
        $response->data = ObjResponse::DefaultResponse();
        try {
            $create = StateCivil::create([
                'name' => $request->name,
    ...
    public function index(Response $response)
     {
        ...
        try {
           ...
           $list = StateCivil::orderBy('id', 'desc')
           ->where('active', 1)
     ...
     public function update(Request $request, Response $response)
     {
         $response->data = ObjResponse::DefaultResponse();
         try {
            $site = StateCivil::find($request->id);
            if ($site) {
     ...
     public function destroy(int $id, Response $response)
     {
         ...
         try {
            ...
            $affectedRows = StateCivil::where('id', $id)
            ->where(function ($query) use ($id) {
                $query->whereNotExists(function ($subquery) use ($id) {
                    $subquery->select(DB::raw(1))
                        ->from('suicidepreventions')
                        ->whereRaw('suicidepreventions.statecivil_id = statecivil.id')
                        ->where('statecivil_id', $id);
                });
            })
            ->update([
        ...
        if ($affectedRows === 0) {
            throw new \Exception('No se puede eliminar ya existe un registro con el estado civil');
        }
