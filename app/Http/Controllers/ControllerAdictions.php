...
use App\Models\Adictions;
use App\Models\ObjResponse;
use Illuminate\Http\Request;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\DB;

class ControllerAdictions extends Controller
{
    public function create(Request $request, Response $response)
    {
        $response->data = ObjResponse::DefaultResponse();
        try {
            $create = Adictions::create([
                'name' => $request->name,
    ...
    public function index(Response $response)
     {
        ...
        try {
           ...
           $list = Adictions::orderBy('id', 'desc')
           ->where('active', 1)
     ...
     public function update(Request $request, Response $response)
     {
         $response->data = ObjResponse::DefaultResponse();
         try {
            $site = Adictions::find($request->id);
            if ($site) {
     ...
     public function destroy(int $id, Response $response)
     {
        ...
        if ($affectedRows === 0) {
            throw new \Exception('No se puede eliminar ya existe un registro con la adicción.');
        }
