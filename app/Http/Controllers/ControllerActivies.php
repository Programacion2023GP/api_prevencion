...
class ControllerActivies extends Controller
{
    ...
    public function index(Response $response)
     {
        ...
        try {
           ...
           $list = activities::orderBy('id', 'desc')
           ->where('active', 1)
     ...
     public function update(Request $request, Response $response)
     {
         $response->data = ObjResponse::DefaultResponse();
         try {
            $site = activities::find($request->id);
            if ($site) {
     ...
     public function destroy(int $id, Response $response)
     {
         ...
         try {
            ...
            $affectedRows = activities::where('id', $id)
            ->where(function ($query) use ($id) {
                $query->whereNotExists(function ($subquery) use ($id) {
                    $subquery->select(DB::raw(1))
                        ->from('suicidepreventions')
                        ->whereRaw('suicidepreventions.activies_id = activities.id')
                        ->where('activies_id', $id);
                });
            })
            ->update([
        ...
        if ($affectedRows === 0) {
            throw new \Exception('No se puede eliminar ya existe un registro con la ocupación.');
        }
