...
class ControllerIndentified extends Controller
{
    ...
    public function index(Response $response)
     ...
     public function destroy(int $id, Response $response)
     {
            ...
            ->where(function ($query) use ($id) {
                $query->whereNotExists(function ($subquery) use ($id) {
                    $subquery->select(DB::raw(1))
                        ...
                        ->whereRaw('suicidepreventions.indetified_id = indentified.id')
                        ->where('indetified_id', $id);
                });
            })
            ->update([
        ...
        if ($affectedRows === 0) {
            throw new \Exception('No se puede eliminar ya existe un registro con el como se indentifica');
        }
