...
class ControllerLiteracy extends Controller
{
    public function create(Request $request, Response $response)
    {
        $response->data = ObjResponse::DefaultResponse();
        try {
            $create = Literacy::create([
                'name' => $request->name,
    ...
    public function index(Response $response)
     ...
     public function destroy(int $id, Response $response)
     {
         ...
         try {
            ...
            $affectedRows = Literacy::where('id', $id)
            ->where(function ($query) use ($id) {
                $query->whereNotExists(function ($subquery) use ($id) {
                    $subquery->select(DB::raw(1))
                        ->from('suicidepreventions')
                        ->whereRaw('suicidepreventions.literacy_id = literacy.id')
                        ->where('literacy_id', $id);
                });
            })
            ->update([
        ...
        if ($affectedRows === 0) {
            throw new \Exception('No se puede eliminar ya existe un registro con el alfabetismo y escolaridad');
        }
